...
- Embedding model choice (local `sentence-transformers` vs API embeddings) — must resolve before Stage 2
- Chunking tuning parameters (T / M / K / N) — empirical, revisit after Stage 1 data exists
- Retention policy + export format — revisit at Stage 5
- Reply trigger policy (commands only vs mention + commands) — revisit at Stage 3
